...
{
    {L"Sound\\explosion.wav", 50, 0},
    {L"Sound\\dig.wav", 50, 0},
    {L"Sound\\largeDig.wav", 50, 0},
    {L"Sound\\flag.wav", 50, 0},
    {L"Sound\\win.wav", 50, 0},
    {L"Sound\\uiClick.wav", 50, 0},
...
bool OpenMciDevice(LPCWSTR _deviceType, LPCWSTR _elementName, UINT& _deviceId)
{
    ...
    DWORD err = mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_ELEMENT, (DWORD_PTR)&openParams);
    if (err != 0) {
        std::wcerr << L"MCI_OPEN failed for " << _elementName << L" with error: " << err << std::endl;
        return false;
    }
...
void PlayMciDevice(UINT _deviceId, bool _repeat)
{
    ...
    DWORD_PTR playFlags = _repeat ? MCI_DGV_PLAY_REPEAT : MCI_NOTIFY; // 0은 기본 재생
